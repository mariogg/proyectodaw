<?php
	...
	class hospedaje{
		...
		function mostrarHospedajeTodos(){
			$consulta="select hospedajes.ID, hospedajes.NOMBRE, hospedajes.DESCRIPCION, hospedajes.PAGINA_WEB, localidad.LOCALIDAD from hospedajes join localidad on localidad.ID_LOCALIDAD = hospedajes.LOCALIDAD";
			if($resultado=$this->conexion->query($consulta)){
				return $resultado;
			}
		}
		...
		function unHospedaje($id){
			$consulta=" select hospedajes.ID, hospedajes.NOMBRE, hospedajes.DESCRIPCION, hospedajes.PAGINA_WEB, localidad.LOCALIDAD from hospedajes join localidad on localidad.ID_LOCALIDAD = hospedajes.LOCALIDAD where ID=".$id;
			if($resultado=$this->conexion->query($consulta)){
				return $resultado;
			}
		}
